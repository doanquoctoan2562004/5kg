// ==UserScript==
// @name         LaymanetBypassTool
// @namespace    http://tampermonkey.net/
// @version      6.4
// @description  Công cụ bypass Laymanet với tính năng mới như Giả mạo domain qua cửa sổ popup + Tự động chạy bypass + Ghi nhớ trạng thái
...
// @run-at       document-end
// @updateURL    https://raw.githubusercontent.com/doanquoctoan256/5kg/main/LaymanetBypassTool-6.4.user.js
// @downloadURL  https://raw.githubusercontent.com/doanquoctoan256/5kg/main/LaymanetBypassTool-6.4.user.js
...
(function() {
    ...
    const styles = `
        ...
        .popup-info {
            ...
            padding: 12px !important; border-radius: 8px !important; margin: 15px 0 !important;
            color: #10b981 !important; font-size: 13px !important; font-family: Consolas, monospace !important;
        }
        .popup-verify-btn {
            ...
            <div class="tool-content">
                <div class="form-group">
                    ...
                    <select id="url-select" class="form-select">
                        ...
                        <!--<option value="china-airline.net">china-airline.net</option>
                        <option value="big-league.cn.com">big-league.cn.com</option>
                        <option value="ruouphongthuy.net">ruouphongthuy.net</option>
                        <option value="suckhoehiendai.com">suckhoehiendai.com</option>
                        <option value="balosieudep.com">balosieudep.com</option> -->
    ...
    function restoreCheckboxStates() {
        try {
            ...
            if (savedAutoFill) {
                updateResult('✅ Đã khôi phục: Tự động điền', 'info');
            }
            if (savedAutoRun) {
                updateResult('🚀 Đã khôi phục: Tự động chạy bypass', 'warning');
            }

        } catch (error) {
            updateResult(`⚠️ Lỗi khôi phục trạng thái: ${error.message}`, 'error');
        }
    }
    ...
    function toggleAutoFill() {
        ...
        updateResult(`${autoFillEnabled ? '✅' : '❌'} Tự động điền: ${autoFillEnabled ? 'BẬT' : 'TẮT'}`, 'info');
    }
    ...
    function toggleAutoRun() {
        ...
        updateResult(`${autoRunEnabled ? '🚀' : '❌'} Tự động chạy: ${autoRunEnabled ? 'BẬT' : 'TẮT'}`,
                    autoRunEnabled ? 'warning' : 'info');
    ...
    function detectImagePattern() {
        try {
            ...
            updateResult(`❌ Lỗi phát hiện pattern: ${error.message}`, 'error');
            return null;
        }
    }
    ...
    async function checkAndRunAutoBypass() {
        ...
        try {
            updateResult('🤖 Đang kiểm tra tự động...', 'warning');

            await new Promise(resolve => setTimeout(resolve, 300));

            const pattern = detectImagePattern();
            if (!pattern) {
                updateResult('⚠️ Không phát hiện được pattern hình ảnh cho auto run', 'warning');
                return;
            }

            const taskUrl = getTaskUrlByPattern(pattern);
            if (!taskUrl) {
                updateResult(`⚠️ Pattern "${pattern}" chưa được hỗ trợ trong auto run`, 'warning');
                return;
            }

            updateResult(`🎯 Auto run phát hiện: ${taskUrl} (pattern: ${pattern})`, 'success');
            ...
            updateResult('⚙️ Đã thiết lập tự động: Facebook + ' + taskUrl, 'info');

            await new Promise(resolve => setTimeout(resolve, 500));

            updateResult('🚀 Bắt đầu bypass tự động...', 'warning');
            await handleProcess();

        } catch (error) {
            updateResult(`❌ Lỗi auto run: ${error.message}`, 'error');
        } finally {
    ...
    function updateResult(text, type = 'info') {
        const timestamp = new Date().toLocaleTimeString();
        const statusClass = type === 'success' ? 'result-success' :
                           type === 'error' ? 'result-error' :
                           type === 'warning' ? 'result-warning' : 'result-info';

        if (resultArea.textContent.includes('Sẵn sàng bắt đầu')) {
            resultArea.innerHTML = `<span class="${statusClass}">[${timestamp}] ${text}</span>`;
        } else {
            resultArea.innerHTML += `\n<span class="${statusClass}">[${timestamp}] ${text}</span>`;
        }
        resultArea.scrollTop = resultArea.scrollHeight;
    }

    function clearResult() {
        resultArea.innerHTML = 'Sẵn sàng bắt đầu...';
    ...
    async function handleCopy() {
        ...
        const lines = resultArea.textContent.split('\n');
        let code = '';

        for (const line of lines) {
            if (line.includes('Thành công:') || line.includes('Bypass Thành Công:')) {
                const match = line.match(/(?:Thành công|Bypass Thành Công): (.+)$/);
                if (match) {
                    code = match[1].trim();
                    break;
                }
...
<body>
    ...
    function showHCaptchaModal(campaignData, config) {
        return new Promise((resolve, reject) => {
            updateResult('Đang mở cửa sổ popup cho hCaptcha...', 'info');
            ...
            modalContent.innerHTML = `
                ...
                <p>Đang mở cửa sổ popup để xác thực domain...</p>
                <div class="popup-info">
                    🌐 Đích: ${config.host}<br>
                    🔑 Sitekey: ${config.hcaptchaSitekey}<br>
                    📋 Phương thức: Giả Mạo Ngữ Cảnh Cửa Sổ Popup
                </div>
            ...
            document.getElementById('popup-start').addEventListener('click', async () => {
                try {
                    updateResult(`Đang mở popup cho ${config.host}...`, 'info');
                    const token = await createPopupWindow(config.hcaptchaSitekey, config.host);

                    document.body.removeChild(modalOverlay);
                    updateResult('✅ hCaptcha hoàn thành qua popup!', 'success');
                    resolve(token);

                } catch (error) {
                    updateResult(`❌ Lỗi popup: ${error.message}`, 'error');
                    document.body.removeChild(modalOverlay);
                    reject(error);
                }
            });
    ...
    async function simulateUserActions(code) {
        try {
            updateResult('🤖 Đang tự động điền form...', 'info');
            ...
            if (!codeInput) {
                updateResult('❌ Không tìm thấy ô nhập mã', 'error');
                return;
            }
            ...
            updateResult('✅ Đã điền mã: ' + code, 'success');
            await new Promise(res => setTimeout(res, 300));

            updateResult('🔍 Đang tìm và check reCaptcha...', 'info');
            ...
            if (recaptchaFrame) {
                updateResult('✅ Tìm thấy reCaptcha, đang thực hiện click...', 'info');
                ...
                if (recaptchaContainer) {
                    ...
                    updateResult('✅ Đã click vào reCaptcha container', 'success');
                } else {
                    recaptchaFrame.click();
                    updateResult('✅ Đã click vào reCaptcha iframe', 'success');
                }
                ...
                try {
                    if (recaptchaFrame.contentDocument) {
                        ...
                        if (checkbox) {
                            checkbox.click();
                            updateResult('✅ Đã click checkbox reCaptcha', 'success');
                        }
                    }
                } catch (e) {
                    updateResult('⚠️ Không thể truy cập trực tiếp vào reCaptcha iframe', 'info');
                }
            } else {
                updateResult('⚠️ Không tìm thấy reCaptcha, tiếp tục với submit', 'info');
            }

            await new Promise(res => setTimeout(res, 300));

            updateResult('🔍 Đang tìm nút xác nhận...', 'info');
            ...
            if (submitButton && !submitButton.disabled) {
                updateResult('✅ Tìm thấy nút xác nhận, đang click...', 'info');
                submitButton.click();
                updateResult('🚀 Đã gửi form thành công!', 'success');
            } else {
                updateResult('⚠️ Không tìm thấy nút xác nhận hoặc nút bị vô hiệu hóa', 'error');
            }

        } catch (error) {
            updateResult(`❌ Lỗi tự động điền: ${error.message}`, 'error');
        }
    }
    ...
    async function performBypass(eurl, platform) {
        ...
        updateResult(`🔧 Đang xử lý ${eurl} trên ${platform}...`, 'info');
        updateResult(`🎯 Đích: ${host}`, 'info');
        updateResult(`🛡️ Phương thức: Giả Mạo Ngữ Cảnh Cửa Sổ Popup`, 'info');

        try {
            updateResult('📡 Đang gửi yêu cầu traffic...', 'info');
            ...
            updateResult('📊 Đang lấy dữ liệu campaign...', 'info');
            ...
            updateResult('✅ Đã lấy dữ liệu campaign', 'success');

            updateResult(`🪟 Đang mở popup hCaptcha cho: ${host}...`, 'info');
            const hCaptchaToken = await showHCaptchaModal(campaignData, config);

            updateResult('🔐 Đang tạo mã bypass...', 'info');
    ...
    async function handleProcess() {
        ...
        try {
            ...
            if (result && result !== 'Không tìm thấy mã trong kết quả trả về.') {
                ...
                updateResult('❌ Không thể tạo mã bypass', 'error');
            }

        } catch (error) {
            updateResult(`❌ Lỗi Bypass: ${error.message}`, 'error');
        } finally {
            isProcessing = false;
    ...
    function initialize() {
        ...
        if (autoRunEnabled) {
            updateResult('⚡ Chế độ tự động đã BẬT - Đang kiểm tra pattern...', 'warning');
        ...
        updateResult('🎉 Công Cụ Bypass Laymanet v6.2 đã sẵn sàng!', 'success');
    }
    ...
    setTimeout(() => {
        if (!resultArea.textContent.includes('v6.2 đã sẵn sàng')) {
            initialize();
        }
    }, 3000);
